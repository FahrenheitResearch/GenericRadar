...
//!
//! This integration test includes `workstation_app/src/settings_ui.rs` by
//! path and compiles the real file, with all of its `#[cfg(test)]` modules,
//! against the same supporting crates the workstation links. The focused
//! compile keeps the settings UI's dependency boundary explicit. The module
//! deliberately references nothing from the `workstation_app` crate itself
//! (only `settings`, `eframe`,
...
//! module's tests, not every public item; the application calls the rest.
