...
Volume: 2 elevations (0.5deg, 1.5deg), 36 rays x 25 bins, four planes.
...
Values are deterministic ramps so the Rust test can assert exact samples:
  DBZH  raw = (ray + bin) % 254 + 1,             u1,  gain 0.5,    offset -32
  VRADH raw = (ray * 2 + bin) % 254 + 1,         u1,  gain 0.1875, offset -24
  WRADH raw = (ray * 1000 + bin * 37) % 65000+1, <u2, gain 2**-12, offset 0
  ZDR   raw = (ray * 811 + bin * 149) % 64000+1, >u2, gain 2**-12, offset -8
The 16-bit gains are 2**-12 so every physical value is exact in f32 (no
assertion tolerance hides a byte-order slip) and the ramps still land in the
physical range each quantity actually occupies.
nodata/undetect are 255/0 for the 8-bit planes and 65535/0 for the 16-bit
ones; gate (0,0)/(0,1) of dataset1 DBZH and of dataset1 WRADH are forced to
nodata/undetect to pin sentinel handling at both storage widths.

The two 16-bit planes exist because uint16 is what several major networks
ship (DWD opendata and FMI both write it) while every real fixture in this
directory is 8-bit or float64, which left the u16 decode path - including
its byte-order branch and its undetect->nodata remap - with no committed
regression pin. They deliberately differ in BYTE ORDER: WRADH is
little-endian, ZDR is big-endian, and their ramps run wide enough that a
byte-swap changes every value rather than a low bit.
...
def main() -> None:
    ...
    with h5py.File(out, "w", libver="earliest") as f:
        ...
        for idx, elangle in enumerate((0.5, 1.5), start=1):
            ...
            wradh = ((ray * 1000 + bin_ * 37) % 65000 + 1).astype(np.uint16)
            zdr = ((ray * 811 + bin_ * 149) % 64000 + 1).astype(np.uint16)
            if idx == 1:
                dbzh[0, 0] = 255  # nodata
                dbzh[0, 1] = 0  # undetect
                wradh[0, 0] = 65535  # nodata
                wradh[0, 1] = 0  # undetect

            for number, (quantity, raw, dtype, gain, offset, nodata, compress) in enumerate(
                (
                    ("DBZH", dbzh, "u1", 0.5, -32.0, 255.0, True),
                    ("VRADH", vradh, "u1", 0.1875, -24.0, 255.0, False),
                    ("WRADH", wradh, "<u2", 2.0**-12, 0.0, 65535.0, True),
                    ("ZDR", zdr, ">u2", 2.0**-12, -8.0, 65535.0, False),
                ),
                start=1,
            ):
                ...
                dset = data.create_dataset("data", data=raw, dtype=dtype, **kwargs)
                ...
                qwhat.attrs.create("nodata", np.float64(nodata))
