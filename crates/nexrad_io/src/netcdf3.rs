...
//! Every count read from the file is checked against a ceiling before it is
//! used to size an allocation, so a corrupt or hostile header cannot turn into
//! a multi-gigabyte reservation.
//!
//! What that ceiling costs, stated plainly, because a dropped file is
//! untrusted input: a classic header describes its arrays in counts, not in
//! bytes it must actually contain, so a small file can honestly ask for a
//! large read. `MAX_NC_ARRAY_BYTES` is the whole of that bound —
//! `numrecs` alone reaches `MAX_NC_DIMENSION_LEN`, so a 540-byte file
//! declaring a 2-byte-per-record variable over 100,000,000 records asks for
//! ~191 MiB from a ~370,000x amplification. Nothing worse is reachable: the
//! request is refused outright above the ceiling, it is made through
//! `try_reserve` so exhausting memory is an `Err` and not an abort, and the
//! read fails at the first byte the file does not contain — measured at 97
//! microseconds for that case. Callers that accept files from the network
//! should size their concurrency against that ceiling rather than against
//! the file size.
...
mod tests {
    ...
    fn tiny_cdf1() -> Vec<u8> {
        ...
        b.extend(8u32.to_be_bytes()); // vsize (3x2 padded to 8)
